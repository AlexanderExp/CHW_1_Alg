...
При небольшом размере массива заметны ощутимые трудности при сортировке массива с большими числами. Так же, можно с легкостью сказать, что данная сортировка сильно опережает предыдущие по скорости работы.

• Асимптотическая сложность - 𝑂 𝑛 + 𝑘 (k – количество различных элементов массива, n – число элементов массива)

• Сортировка выполняется только для целых чисел

• Целесообразно применять, когда диапазон чисел небольшой по сравнению с размером сортируемого массива (Это отчетливо видно из первого графика по Selection Sort)

• Требуется дополнительная память - 𝑂 𝑛 + 𝑘 в случае устойчивой


5) Selection Sort (Stable)
...
Видим огромное преимущество этой сортировки при работе с массивом, заполненным маленькими значениями, от 0 до 5(включая обе границы).

Можем отметить 3 заметных скачка на втором графике по Radix Sort и 1 на первом. Видимо, в этот момент процессор озадачили сторонние процессы.

В общем,
В лучшем случае, когда все элементы состоят из одинакового количества цифр - O(a*(n+b)) (n - количество элементов в массиве, каждая цифра принимает значения от 0 до b, a - количество разрядов в числе). Если количество разрядов - константа, а b = O(n), то сложность O(n);

В среднем случае - O(m*T(n)), где m - количество разрядов, а T(n) - время работы устойчивой сортировки.
...
Особенностей не наблюдается. Очень неплохие результаты по времени. 

• Имеет доказанную оценку худшего случая O(n logn)
...
Значительно дольше работает на массиве со случайными числами от 0 до 4000(включая обе границы), чем на массиве, отсортированном в порядке убывания и на почти отсортированном массиве.

В целом, результаты довольно стабильные, но наблюдается несколько скачков.

В лучшем, худшем, среднем случае - O(n*log(n)) по времени и O(n) по памяти. n - размер массива.
...
Наблюдаем, что в случаях с массивами со значениями от 0 до 4000 и от 0 до 5 результаты прекрасны.

В лучшем, среднем случае - O(n*log(n)), в худшем -O(n) по времени и O(n*log(n)) по памяти. n - размер массива.
...
Алгоритм невероятно хорошо справляется с сортировкой почти отсортированного массива. Хуже всего этой сортировке дается обработка массива, отсортированного в обратном порядке.

В худшем и среднем случаях - O(n^2) по времени

В лучшем - O(n) по времени
...
Сложность вставки элемента в нужную позицию теперь O(log(n)). Видим(особенно на большом массиве), что время сортировки сильно сократилось, что здорово ! 
...
Видим, что сортировка прекрасно справляется с массивом, заполненным значениями от 0 до 5 (включая обе границы). Худший массив для этой сортировки - массив со значениями от 0 до 4000 (включая обе границы)

Сортировка работает за O(n*log(n)^2) в худшем случае
...
С последовательностью Циура алгоритм сортировки Шелла работает значительно быстрее на почти отсортированном массиве. В целом, работает быстрее, но сильно замедляется при обработке больших чисел. 

# Теперь, общий вывод:

1) На массиве со случайными значениями от 0 до 5 картина следующая:

<img width="809" alt="image" src="https://user-images.githubusercontent.com/95678672/219972582-f08972e1-2b8e-4a3e-a4c2-31bc51e6406b.png"> <img width="1109" alt="image" src="https://user-images.githubusercontent.com/95678672/219965646-97f50b40-7a0c-40e4-9a56-bb92f411be6b.png">

Худшие сортировки для массива, заполненного маленькими значениями: Сортировка вставками, пузырьком, пузырьком с первым условием Айверсона, пузырьком с первым и вторым условиями Айверсона. Так же довольно плохо работает сортировка выбором.

Лучшие: Radix и Quick сортировки.

2) На массиве со случайными значениями от 0 до 4000 картина следующая:

<img width="814" alt="image" src="https://user-images.githubusercontent.com/95678672/219972681-688c4bc8-aadd-4112-ae5d-113b23da37ab.png"> <img width="1109" alt="image" src="https://user-images.githubusercontent.com/95678672/219965657-26100877-6566-402f-8615-01da71590ad0.png">

Худшие сортировки для массива, заполненного большими значениями: Сортировка вставками, пузырьком, пузырьком с первым условием Айверсона, пузырьком с первым и вторым условиями Айверсона. Так же довольно плохо работает сортировка выбором.

Лучшие: Сортировка подсчетом, быстрая сортировка.

3) На почти отсортированном массиве:

<img width="814" alt="image" src="https://user-images.githubusercontent.com/95678672/219972710-df696300-bb0a-4105-854c-f2040977ef95.png"> <img width="1109" alt="image" src="https://user-images.githubusercontent.com/95678672/219965675-c5c07f1f-2a8a-4621-bfab-3990d9b6a8c6.png">

Худшие сортировки для массива, заполненного маленькими значениями: пузырьком,  пузырьком с первым и вторым условиями Айверсона, быстрая сортировка, сортировка выбором.

Лучшие: Все остальные. Самая лучшая - подсчётом.

4) На массиве, отсортированном в убывающем порядке:

<img width="826" alt="image" src="https://user-images.githubusercontent.com/95678672/219973110-f3cbee8f-ae4d-42e3-b532-1eaae30cb5f1.png"> <img width="1109" alt="image" src="https://user-images.githubusercontent.com/95678672/219965693-12601f24-5f49-44bc-9215-d850b6a4d684.png">

Худшие сортировки для массива, заполненного маленькими значениями: сортировка вставками, пузырьком, пузырьком с первым условием Айверсона, пузырьком с первым и вторым условиями Айверсона. Чуть лучше, но всэ равно плохо работают сортировки: быстрая и выбором.
...
Для массива с небольшими значениями стоит выбрать быструю сортировку или Radix sort

Для массива с большими значениями стоит выбрать быструю сортировку или сортировку подсчётом

Для почти отсортированного массива лучше пользоваться сортировкой подсчётом

Для отсортированного в обратном порядке массива лучше использовать сортировку подсчетом или Radix sort
